...
public class GuiLogin extends GuiScreen {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        if (!AnimationUtils.animationDone) {
            ...
            if (fontLoaded) {
                FontManager.robotoL18.drawCenteredStringWithAlpha("Cracked by ho4", width / 2, height / 2,
                        ColorUtils.WHITE.c, this.font_animation > 1 ? 1 : this.font_animation);
            }
        }
    ...
    protected void keyTyped(char typedChar, int keyCode) throws IOException {
     ...
     //       IRCClient.isChina = !IRCClient.isChina;
            Display.setTitle(Flux.NAME + " b" + Flux.VERSION + " - " );
            this.dialog = new PopupDialog("Server Switched", String.format("Successfully switched server to %s server!", "Global"), 200, 55, true);
        }
